...
class RotaryEncoderEvent:
    ...
    TURN_RIGHT_SUPER_FAST = 14
    TURN_LEFT_SUPER_FAST = 15
        ...
        if __e_l_d > 0:
            if __e_l_d > fast_turn_count:
                if self._sw_held_with_encoder:
                    ...
                    if __e_l_d > 4 * fast_turn_count:
                        self._flag_last_event = RotaryEncoderEvent.TURN_RIGHT_SUPER_FAST
            else:
                if self._sw_held_with_encoder:
            ...
            if __e_l_d < -fast_turn_count:
                if self._sw_held_with_encoder:
                    ...
                    if __e_l_d < -4 * fast_turn_count:
                        self._flag_last_event = RotaryEncoderEvent.TURN_LEFT_SUPER_FAST
            else:
                if self._sw_held_with_encoder:
